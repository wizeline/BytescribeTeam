...
def cosine(a, b):  # assume normalized
    return float(np.dot(a, b))


def pick_images_for_bullets(bullets: list[str], images: list[dict]):
    # Precompute image embeddings from captions/tags/title
    for img in images:
        text = " | ".join([img.get("title", ""), img.get("caption", "")] + img.get("tags", []))
        img["emb"] = normalize(titan_embed(text))

    results = []
    for b in bullets:
        b_emb = normalize(titan_embed(b))
        best = max(images, key=lambda im: cosine(b_emb, im["emb"]))
        results.append({
            "text": b,
            "image_url": best["s3_url"],
            "match_score": cosine(b_emb, best["emb"])
        })
    return results


# Generate bullets
def summarize_to_bullets(article_text: str, num_bullets=3):
    prompt = f"""Summarize the article into {num_bullets} non-overlapping bullet points (≤60 words each).
